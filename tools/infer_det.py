...
def draw_det_res_and_label(dt_boxes, classes, config, img, img_name, save_path):
    ...
    if len(dt_boxes) > 0:
        ...
        for box in dt_boxes:
            ...
            src_im = cv2.putText(src_im, labels[classes[index]], (box[0][0][0], box[0][0][1]), font, 1, (255, 0, 0), 2)
            index += 1
        if not os.path.exists(save_path):
            os.makedirs(save_path)
        save_path = os.path.join(save_path, os.path.basename(img_name))
        ...
        for file in get_image_file_list(config['Global']['infer_img']):
            ...
            if isinstance(post_result, dict):
                ...
                for k in post_result.keys():
                    ...
                    # draw_det_res_and_label(boxes, classes, config, src_img, file, save_det_path)
            else:
                boxes = post_result[0]['points']
                texts = post_result[0]['classes']
                dt_boxes_json = []
                # write result
                for box in boxes:
                    tmp_json = {"transcription": ""}
